...
**Integration test modules** (`tests/manual/tests/`): `sanity` (read-only protocol), `smoke` (balances/categories), `account`, `category-group`, `category`, `payee`, `transaction`, `budget`, `notes`, `rules`, `schedule`, `tags` (#451), `batch_uncategorized_rules_upsert`, `entrypoint`, `advanced` (bank sync, raw SQL).
...
**The stdio leg is ON and GATING since #423.** #383 landed the infrastructure (fixture, host-side client, project, named volume) but the leg could not start on a strict Playwright, and once that was fixed four tests in the write-heavy block failed on Actual's rate limiter. #423 fixed both. The reporter path moved to `playwright-report/docker-e2e` (the HTML report had been nested inside `test-results`, Playwright's own `outputDir`, which the reporter clears before writing). And **the stdio leg now has its OWN Actual server**, `actual-budget-stdio-test`, with its own bootstrap and its own sync-id volume, so the two legs no longer share a 500-requests-per-minute window. That was the actual cause: a throttled `api.sync()` makes upstream CLOSE the budget (a #396 close-before-load), the next operation needs a re-download and therefore a fresh login, and that login is refused inside the same full window, which `withAuthRetry` backs off 25s and then fails. The pacer could not prevent it, because the overflow is request amplification rather than call rate: the host-side leg never reaches the call budget. `STDIO_COOLDOWN_S` therefore defaults to 0 now and is kept only in case the legs ever share a server again. `RUN_STDIO_E2E=false` skips the leg; `STDIO_E2E_GATING=false` downgrades it to advisory for a deliberately dirty local run.

**Two env vars are REQUIRED for the stdio leg and neither can be inherited**, which is worth knowing before debugging a 40ms failure: `docker exec` inherits the container's CONFIGURED environment, and that names the HTTP leg's server and holds no sync id at all (the compose ENTRYPOINT `export`s it from a file into its own shell). So `run-docker-e2e.sh` passes `MCP_STDIO_SERVER_URL` (to `-e ACTUAL_SERVER_URL`) and `MCP_STDIO_SYNC_ID_FILE` (`/tmp-stdio/actual-sync-id.txt`, a read-only mount of the stdio bootstrap's volume). The stdio budget cache stays on its own named volume, `mcp-test-stdio-data`: two api instances must never share a budget cache, and that is doubly true now they point at different servers.

Four tests are HTTP-only, each with the reason at the test: the two `actual_session_*` tools (stdio has one implicit session, so there is nothing to list or expire), and the two that read the raw JSON-RPC envelope through `mcp.post` (the SDK surfaces a tool error as an exception instead). So the stdio project skips exactly those four: read the totals from `npx playwright test --list` rather than pinning them (the stdio project collects the same set as the HTTP one and skips exactly those four). The transports run SEQUENTIALLY, and the E2E wall clock roughly doubles. Since #423 that is no longer about the rate limiter (each leg has its own Actual server now); they still share this host's CPU and the ONE MCP container, and a parallel run would make a failure in either leg hard to attribute. **The pacer does NOT span the two legs.** `pace()` in `tests/shared/e2e-helpers.ts` is module scoped, so it shares one window only WITHIN a process, and the two legs are SEPARATE processes (HTTP inside the runner container, stdio on the host). Each leg paces itself. The 75s cool-down that used to sit between them (`STDIO_COOLDOWN_S`) defaulted to 75 because the legs shared one server and therefore one 60-second window: an earlier version claimed they shared a PACER, and CI proved otherwise (the stdio leg's first login refused with "Too many requests", 39 worker restarts, 377 errors from one refusal). #423 gave stdio its own server, so there is no shared window left to drain and the knob defaults to 0, kept only in case they ever share a server again. The reason the leg itself now fits under the ceiling is #419: since v0.16.13 a stdio process logs in ONCE and keeps the singleton alive, so ~100 tool calls are ~2 requests each rather than a full init/download/op/sync per call. stdio is additionally covered by the dual-transport gate (#280) and the framing check (#323).
...
| `src/lib/init-failure.ts` | #438 classified a failed Actual connection into a CLOSED enum plus a fixed sentence, so no upstream string (a stack, `SyncError.meta.query`, an `EACCES /home/<user>/...` path, the server URL) reaches a client. #452 moved it out of `httpServer.ts`, which re-exports it, because stdio needed the same answers. It also owns the `markInitFailure` / `isInitFailure` BRAND: the classifier's last-resort branches match message PROSE, which is safe only where the caller already knows it holds an init failure. stdio's catch sees every tool error, where a #270 op timeout reads as `timeout` and a #422 rate limit as `auth_failed`, so stdio classifies ONLY branded errors. The mark is applied at the two chokepoints a failure can come from: the api-init try/catch in `initActualApiForOperation`, and both branches of `loadBudgetTracked`'s catch, which is the funnel every budget load passes through (the write drain and the pooled precondition reach a load WITHOUT passing through the adapter's init, so branding only there missed them) |
| `src/lib/server-version-guard.ts` | #276: advisory warning (once) when the Actual Budget SERVER version is outside the range this build's `@actual-app/api` is known to work with. #439 added a third branch, LAST in the chain so the existing ones keep precedence: the server is ahead of the api this build bundles, compared on MAJOR.MINOR only and silent when equal or behind. #453 then added a SECOND call site, at the top of `loadBudgetTracked`, so the warning also fires BEFORE the download rather than only after a successful op. That was the point: a server far enough ahead that the download itself fails could never reach the post-op site, which is the #427 case. The post-op site remains for a process that never downloads a budget. #438 still owns surfacing the real error after a failure, and since #452 its sentence reaches stdio too |
